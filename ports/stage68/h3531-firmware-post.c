...
static unsigned read_mem_mb(void)
{
    FILE *f;
    char line[128];
    unsigned long kb = 0;
    ...
    fclose(f);
    return (unsigned)(kb / 1024UL);
}

static void info_line(struct fbctx *fb, int y, const char *name, const char *value, uint32_t white)
{
    char line[112];
    snprintf(line,sizeof(line),"%-18s %s",name,value);
...
{
    clearfb(fb,black);
    draw_text(fb,48,30,3,"STAYPLAYTION FIRMWARE",white);
}
...
static int run_menu(struct fbctx *fb, int timeout_ms, uint32_t black, uint32_t white)
{
    ...
    draw_text(fb,48,68,2,"HI3531 HARDWARE POST",white);
    ...
    mem_mb = read_mem_mb();
    if (mem_mb)
        snprintf(memory,sizeof(memory),"%u MB",mem_mb);
    else
    ...
    status_line(fb,280,"X11 RUNTIME",x_ok,white);
    status_line(fb,310,"DESKTOP CORE",desktop_ok,white);
    status_line(fb,340,"USB KEYBOARD",kn>0,white);
    status_line(fb,370,"USB MOUSE",mouse,white);
    status_line(fb,400,"RESCUE MONITOR",monitor_ok,white);

    if(!storage_ok || !x_ok || !desktop_ok || !monitor_ok) critical=1;

    if(critical) {
        draw_text(fb,48,460,2,"CRITICAL PREFLIGHT FAILURE",white);
        draw_text(fb,48,490,2,"STARTING RESCUE MONITOR",white);
        sleep(2);
        rc=10;
    } else {
        draw_text(fb,48,460,2,"F2  RESCUE MONITOR",white);
        draw_text(fb,48,490,2,"AUTO START DESKTOP",white);
        draw_text(fb,48,520,2,"FIRMWARE CHECKS COMPLETE",white);
        rc=wait_f2(kfds,kn,timeout_ms);
        if(rc==2) {
            draw_text(fb,48,570,2,"F2 PRESSED - STARTING MONITOR",white);
            usleep(500000);
        } else {
            draw_text(fb,48,570,2,"STARTUP SELECTION COMPLETE",white);
            usleep(500000);
        }
    }
...
{
    draw_header(fb,black,white);
    draw_text(fb,48,88,2,"FIRMWARE COMPATIBILITY PREINIT",white);
...
static void run_ready(struct fbctx *fb, int timeout_ms, uint32_t black, uint32_t white)
{
    force_opaque(fb);
    draw_header(fb,black,white);
    draw_text(fb,48,105,2,"FIRMWARE HARDWARE READY",white);
