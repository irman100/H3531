...
/* Aspect-correct SNES 4:3 viewport: 256x224 -> 896x672.
 * The generic nearest-neighbour map for 896/256 is exactly 7/2, so each
 * pair of source pixels expands as 4 copies + 3 copies. This preserves the
 * exact current picture geometry while avoiding map lookups on every pixel. */
static bool h3531_present_rgb565_snes_7over2x3(h3531_fb_t *h,
      const uint8_t *src, unsigned src_pitch,
      unsigned dx, unsigned dy)
{
   unsigned sy;
   if (!h3531_reserve_ps1_row(896U))
      return false;
   ...
   for (sy = 0; sy < 224U; ++sy)
   {
      const uint16_t *srow = (const uint16_t*)(src + (size_t)sy * src_pitch);
      unsigned pair;
      for (pair = 0; pair < 128U; ++pair)
      {
         const uint16_t p0 = h3531_fast_rgb565_to_a1r5g5b5(srow[pair * 2U + 0U]);
         const uint16_t p1 = h3531_fast_rgb565_to_a1r5g5b5(srow[pair * 2U + 1U]);
         const unsigned ox = pair * 7U;
         h3531_row[ox + 0U] = p0;
         h3531_row[ox + 1U] = p0;
         h3531_row[ox + 2U] = p0;
         h3531_row[ox + 3U] = p0;
      ...
      }
      {
         ...
         for (rep = 0; rep < 3U; ++rep)
         {
            uint16_t *dst = (uint16_t*)(h->mem +
                  (size_t)(dy + sy * 3U + rep) * h->stride) + dx;
            memcpy(dst, h3531_row, 896U * sizeof(*h3531_row));
         }
      }
   }
#if defined(__arm__)
...
static bool h3531_stage426_snes_43_logged = false;
...
src = src.replace(reset_anchor, reset_anchor +
      ...
      '   h3531_stage426_snes_43_logged = false;\n', 1)
