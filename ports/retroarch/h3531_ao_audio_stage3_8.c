/* H3531 AO audio driver for RetroArch Stage 3.8.
 *
 * Standard integration policy:
 *   - implements RetroArch audio_driver_t;
 *   - receives native-endian S16 stereo PCM;
 *   - downmixes to the physically proven H3531 AO S16 mono transport;
 *   - keeps AO5/ch0, 48 kHz, 160-sample/320-byte frame ABI unchanged;
 *   - uses blocking write() as the pacing path;
 *   - intentionally leaves wait_writable NULL so RetroArch uses its normal
 *     inline audio path instead of starting the threaded audio pipeline.
 */
      ...
      uint32_t *total, uint32_t *free_blocks, uint32_t *busy_blocks)
{
   ...
   ctx->last_total = st.total_blocks;
   ctx->last_free  = st.free_blocks;
   ctx->last_busy  = st.busy_blocks;
   if (total) *total = st.total_blocks;
   if (free_blocks) *free_blocks = st.free_blocks;
   if (busy_blocks) *busy_blocks = st.busy_blocks;
   return 0;
}
...
static int h3531_send_block(h3531_audio_t *ctx)
{
   ...
   if ((ctx->blocks_sent % 300U) == 0U)
      RARCH_LOG("[H3531 AO] STATS blocks=%llu input_frames=%llu writes=%llu waits=%llu free=%u busy=%u sendmax_us=%u query_fail=%llu\n",
            ...
            ctx->last_free, ctx->last_busy, ctx->send_max_us,
      ...
      unsigned rate, unsigned latency, unsigned *new_out_rate)
{
   ...
   RARCH_LOG("[H3531 AO] ready-inline: requested=%uHz output=48000Hz latency=%ums S16 stereo->mono AO5/ch0 30x160 proven-frame=320B\n",
         rate, latency);
   return ctx;

error:
   RARCH_ERR("[H3531 AO] init failed rc=%d (0x%08x) errno=%d\n", rc, (unsigned)rc, errno);
   h3531_disable(ctx);
...
static void h3531_audio_free(void *data)
{
   h3531_audio_t *ctx = (h3531_audio_t*)data;
   if (!ctx) return;
   RARCH_LOG("[H3531 AO] final-inline: blocks=%llu input_frames=%llu writes=%llu waits=%llu send_fail=%llu query_fail=%llu sendmax_us=%u\n",
         (unsigned long long)ctx->blocks_sent,
         (unsigned long long)ctx->input_frames,
         (unsigned long long)ctx->write_calls,
         (unsigned long long)ctx->waits,
...
static size_t h3531_audio_write_avail(void *data)
{
   h3531_audio_t *ctx = (h3531_audio_t*)data;
   uint32_t total = 0, free_blocks = 0, busy_blocks = 0;
   size_t capacity_frames = (size_t)H3531_AO_QUEUE_BLOCKS * H3531_AO_SAMPLES;
   size_t frames;
   ...
   if (ctx->mono_fill && ctx->mono_fill < H3531_AO_SAMPLES)
...
audio_driver_t audio_oss = {
   ...
   NULL, /* write_raw */
   NULL, /* wait_writable: NULL forces RetroArch standard inline/blocking path */
